   ...
   // ---------------------- sanitize_input_data ----------------------------
	...
	public function add_new_user($data){
		...
		try{
			...
			elseif(strlen($p)<=5){
			...
			}elseif($user_permission==0){
				$user_faculty = $this->sanitize_input_data($data['faculty']);
				$user_sem = $this->sanitize_input_data($data['semester']);

				if ($user_faculty!=NULL && $user_sem!=NULL) {
				$add_user = $this->db->prepare("INSERT INTO swastik_users_tbv (user_fullname, user_email, user_password, user_permission, user_code,user_faculty, user_semester) VALUES (:x, :y, :z, :a, :b, :c, :d) ");
				...
				header('Location: success.php');	
					...
					$message = "Input Error".$user_faculty;

				}

			}
				else{
				$add_user = $this->db->prepare("INSERT INTO swastik_users_tbv (user_fullname, user_email, user_password, user_permission, user_code) VALUES (:x, :y, :z, :a, :b) ");
				$add_user->bindparam(':x', $user_fullname);
				$add_user->bindparam(':y', $user_email);
				$add_user->bindparam(':z', $user_password1);
				$add_user->bindparam(':a', $user_permission);
...
// --------------------------Login Validation-------------------------
        ...
        try
       {
          ...
          if($stmt->rowCount() > 0)
          {
          	if($userRow['isactive']!=1){
          		...
          		session_start();
          		$sess = md5(sha1(rand(10,1000).$userRow['user_email']));
	...
	public function post_assi($data, $file){
		...
		$hash = md5(sha1(rand(1,100000).$filename.$_SESSION['user_id']));
	...
	public function upload_note($data, $file){
		...
		$hash = substr(md5(sha1(rand(1,100000).$filename.$_SESSION['user_id'])),0,5);
		$tempname = $file["tmp_name"];	
...
//--------------Feedback------------------------------
	...
	public function post_feedback($data){
		...
		try{
			...
			$post_feedback->bindparam(':b', $user);
...
try{
$add_user = $this->db->prepare("INSERT INTO swastik_users (user_fullname, user_email, user_password, user_permission, user_code,user_faculty, user_semester) VALUES (:x, :y, :z, :a, :b,:c,:d) ");
				...
				$add_user->bindparam(':b', $r['user_code']);
...
public function changeImage($file){
		...
		$tempname = $file["tmp_name"];	
		if($file['name'] != NULL){
		$hash = md5(sha1(rand(10,1000).$userRow['user_email']));
